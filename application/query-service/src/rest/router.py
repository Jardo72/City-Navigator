...
request_duration_histogram = Histogram(
    ...
    labelnames=["method", "path"],
    buckets=[0.25, 0.5, 0.75, 1, 1.5, "+Inf"]
)
...
async def get_station_details(name: str, db: Session = Depends(get_db)):
    ...
    with request_duration_histogram.labels(method="GET", path="/station").time():
...
async def get_line_details(label: str, db: Session = Depends(get_db)):
    ...
    with request_duration_histogram.labels(method="GET", path="/line").time():
...
async def search_journey_plan(start: str, destination: str, db: Session = Depends(get_db)):
    ...
    with request_duration_histogram.labels(method="GET", path="/journey-plan").time():
