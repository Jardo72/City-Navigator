...
| `gradual-load-peak-cfg.json` | Peak mixed load with gradual ramp-up — 9 journey-plan threads, 2 station-query threads, 2 line-query threads, 2 station-filter threads; no intentional errors; 50-minute main phase preceded by a ramp-up of 15 seconds between each of the 15 workers; each worker also starts with a 0.3 s per-request sleep that decreases by 0.001 s per request until it reaches zero |
